...
adminRouters.route("/admin").get(async (req, res) => {
  try {
...
adminRouters.route("/admin/:id").get(async (request, response) => {
    try {
        ...
        const db = database.getDb();
        const data = await db.collection("admins").findOne({ _id: new ObjectId(adminId) });
...
adminRouters.route("/admin").post(async (request, response) => {
    try {
        ...
        const data = await db.collection("admins").insertOne(mongoObject);
...
adminRouters.route("/admin/:id").put(async (request, response) => {
    try {
        ...
        const data = await db.collection("admins").updateOne({ _id: ObjectId(request.params.id) }, mongoObject);
...
adminRouters.route("/admin/:id").delete(async (request, response) => {
    try {
        const db = database.getDb();
        const data = await db.collection("admins").deleteOne({ _id: ObjectId(request.params.id) });
